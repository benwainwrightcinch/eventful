...
it("executes the correct handler", () => {
  ...
  const handlerOne = (_key: "foo", _event: EventBridgeEvent<"foo", Thing>) =>
    fail("Handler one was not supposed to be called");

  const handlerTwo = (_key: "fo2o", _event: EventBridgeEvent<"fo2o", Thing>) =>
...
it("errors at build time if the type is mismatched from the key in the handler event", () => {
  ...
  const handlerOne = (_key: "foo", _event: EventBridgeEvent<"foo", Thing3>) =>
    undefined;
  const handlerTwo = (_key: "fo2o", _event: EventBridgeEvent<"fo2o", Thing>) =>
    undefined;
  const handlerThree = (
...
it("compiles without errors when state is supplied", () => {
  const mockEvent: IncomingEvent = {
    id: "123",
    version: "a",
    account: "a",
